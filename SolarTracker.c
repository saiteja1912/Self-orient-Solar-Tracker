...
{ 
  ...
  Serial.begin(9600);
  pinMode(5, OUTPUT);  //led for output
  pinMode(6, OUTPUT);   //led for output
}  
...
{ 
  ...
  if((diff1 <= error) || (diff2 <= error)) {
    ...
    if(R1 > R2)
    {
      initial_position = initial_position-2;  //Move the servo towards 0 degree
      //Serial.println(initial_position);
      Serial.println(R1);
      Serial.println(R2);
      digitalWrite(5, HIGH); 
    ...
    if(R1 < R2) 
    {
      initial_position = initial_position+2; //Move the servo towards 180 degree
      Serial.println(R1);
      Serial.println(R2);
      digitalWrite(6, HIGH); 
      digitalWrite(5, LOW); 
